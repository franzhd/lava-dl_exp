...
class Network(torch.nn.Module):
    def __init__(self, input, output, treshold, voltage_decay):
        ...
        self.blocks = torch.nn.ModuleList([
                ...
                slayer.block.cuba.Dense(neuron_params_drop, 256,256, weight_norm=True, delay=True),
...
def main():
    ...
    parser.add_argument('--config', type=str, help='nome del config file per creare la cartella adeguata')
    args = parser.parse_args()

    params = nni.get_next_parameter()

    os.chdir(f'{Path.home()}/lava-dl_experiment/nni_results/{args.config}/{nni.get_experiment_id()}/trials/{nni.get_trial_id()}')
    ...
    dataset = WISDM_Dataset_parser(f'{Path.home()}/lava-dl_experiment/data/watch_subset2_40.npz')
    ...
    net = Network(6, 7, params['treshold'], params['voltage_decay']).to(device)
    ...
    for epoch in range(epochs):
        ...
        if count < 4:
            ...
            if stats.testing.best_accuracy:
                ...
                summed_matrix = np.sum(outputs, axis=-1)
                divided_matrix = summed_matrix / outputs.shape[1]

                predictions = np.argmax(divided_matrix, axis=1)
